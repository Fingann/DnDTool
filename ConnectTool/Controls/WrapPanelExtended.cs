...
//http://stackoverflow.com/questions/11636396/set-max-rows-in-a-wrap-panel/41953520#41953520

namespace DnDTool.Controls
{
    using System.Windows;
    using System.Windows.Controls;

    class WrapPanelExtended : WrapPanel
    {
        public static readonly DependencyProperty RowsOrColumnsCountProperty =
            DependencyProperty.Register(
                "RowsOrColumnsCount",
                typeof(int),
                typeof(WrapPanelExtended),
        ...
        public int RowsOrColumnsCount
        {
            get
            {
                return (int)GetValue(RowsOrColumnsCountProperty);
            }
            set
            {
                SetValue(RowsOrColumnsCountProperty, Math.Max(value, 1));
            }
        }
        ...
        protected override Size MeasureOverride(Size availableSize)
        {
            if (Children.Count > 0)
            {
                ...
                {
                    var suitableWidth = EstimateSuitableRowOrColumnLength(
                        Children.Cast<UIElement>(),
                        true,
                        availableSize,
                        RowsOrColumnsCount);

                    newAvailableSize = double.IsNaN(suitableWidth) || suitableWidth <= 0
                                           ? availableSize
                                           : new Size(
                                                 Math.Min(suitableWidth, availableSize.Width),
                                                 availableSize.Height);
                }
                else
                {
                    var suitableHeigth = EstimateSuitableRowOrColumnLength(
                        Children.Cast<UIElement>(),
                        false,
                        availableSize,
                        RowsOrColumnsCount);
                    newAvailableSize = double.IsNaN(suitableHeigth) || suitableHeigth <= 0
                                           ? availableSize
                                           : new Size(
                                                 availableSize.Width,
                                                 Math.Min(suitableHeigth, availableSize.Height));
                }
        ...
        private double EstimateSuitableRowOrColumnLength(
            ...
            int rowsOrColumnsCount)
        {
            ...
            var desiredLengths =
                elementsList.Select(el => DesiredLength(el, availableSize, trueRowsFalseColumns)).ToList();
            ...
            var desiredLengthsRepaired =
                desiredLengths.Select(length => double.IsNaN(length) ? maxLength : length).ToList();
            ...
            while (CountRowsOrColumnsNumber(desiredLengthsRepaired, suitableRowOrColumnLength, out nextLengthIncrement)
                   > maxCount)
            {
        ...
        private int CountRowsOrColumnsNumber(
            List<double> desiredLengths,
            double rowOrColumnLengthLimit,
            out double nextLengthIncrement)
        {
